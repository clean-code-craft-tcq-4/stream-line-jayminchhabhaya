...
TEST_CASE("TEST_CASE 1 :: Generate Parameters of Battery Temperature") {
	...
	assert(printoutputtocommunicate(batteryobj.temperaturedata,batteryobj.socdata) == true);
}
...
TEST_CASE("TEST_CASE 3 :: Fail to Generate Parameters of Battery") {
    ...
    Battery batteryobj;
    std::srand(static_cast<unsigned int>(std::time(nullptr))); 
	assert(printoutputtocommunicate(batteryobj.temperaturedata,batteryobj.socdata) == false);
}